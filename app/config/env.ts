...
export function loadPublicEnv(): { env: PublicEnv; dotenvFile: string } {
  ...
  const apiBaseURL = must(process.env.EXPO_PUBLIC_API_BASE_URL, "EXPO_PUBLIC_API_BASE_URL", dotenvFile);
  const tokenBaseURL = must(process.env.EXPO_PUBLIC_TOKEN_BASE_URL, "EXPO_PUBLIC_TOKEN_BASE_URL", dotenvFile);
