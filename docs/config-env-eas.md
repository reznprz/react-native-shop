📄 Config + Env + EAS Build Notes (Why dotenv-cli fixed it)
Problem we hit
...
CI builds “worked locally” but failed in GitHub Actions / EAS.
...
EAS may spawn subprocesses in a way where env vars aren’t inherited as expected in some contexts
...
So EAS doesn’t need to successfully read the file later — it just sees the env values already set.

✅ This is why dotenv-cli made yarn build:prod:android succeed.

Our recommended “enterprise-stable” pattern
...
This makes builds consistent and avoids “DOTENV_FILE not respected” surprises.
...
✅ dotenv-cli fixed the issue because it injects env vars early, so EAS doesn’t need to parse .env.* later.

✅ Relying only on DOTENV_FILE is fragile in CI/EAS because subprocesses and file parsing can fail in subtle ways.
